...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0library_${CC_NAME}_ccaas  \
                  --network supplychain \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0deliver_${CC_NAME}_ccaas \
                  --network supplychain \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run --rm -d --name peer0producer_${CC_NAME}_ccaas  \
                  --network supplychain \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest                
    ...
    infoln "    docker run --rm -d --name peer0library_${CC_NAME}_ccaas  \
                  --network supplychain \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.library and peer0.deliver
infoln "Installing chaincode on peer0.library..."
installChaincode 1
infoln "Install chaincode on peer0.deliver..."
installChaincode 2
...
## approve the definition for library
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect library to have approved and deliver not to
checkCommitReadiness 1 "\"LibraryMSP\": true" "\"DeliverMSP\": false" "\"ProducerMSP\": false"
checkCommitReadiness 2 "\"LibraryMSP\": true" "\"DeliverMSP\": false" "\"ProducerMSP\": false"
checkCommitReadiness 3 "\"LibraryMSP\": true" "\"DeliverMSP\": false" "\"ProducerMSP\": false"

## now approve also for deliver
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"LibraryMSP\": true" "\"DeliverMSP\": true" "\"ProducerMSP\": false"
checkCommitReadiness 2 "\"LibraryMSP\": true" "\"DeliverMSP\": true" "\"ProducerMSP\": false"
checkCommitReadiness 3 "\"LibraryMSP\": true" "\"DeliverMSP\": true" "\"ProducerMSP\": false"

approveForMyOrg 3

checkCommitReadiness 1 "\"LibraryMSP\": true" "\"DeliverMSP\": true" "\"ProducerMSP\": true"
checkCommitReadiness 2 "\"LibraryMSP\": true" "\"DeliverMSP\": true" "\"ProducerMSP\": true"
checkCommitReadiness 3 "\"LibraryMSP\": true" "\"DeliverMSP\": true" "\"ProducerMSP\": true"
